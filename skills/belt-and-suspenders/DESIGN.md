# Belt and suspenders — design and rationale

This is the former body of `SKILL.md`, moved here by harmonic-forge#659. The
skill no longer tells lanes to read it: arming is `python3
...
- **Arm the canonical command once.** There is exactly one argument set per
  `LANE` value, and the script refuses anything else, printing the command to
...
under the belt; there is no second list to remember. The manifest also carries
each repo's checkout path, which a convention like `<dir>/<repo name>` cannot:
hrse's checkout is `HRSE2`, and harmonic-forge's is `~/harmonic-forge` while its
lane worktrees live in `~/Harmonic_Projects/`.

This is the correction to a first attempt that hardcoded four repos and wrote a
test asserting that list (harmonic-forge#596). Naming them fixed the instance —
the belt did cover four repos — and left the class: *"which repos?"* stayed a
question someone had to keep answering correctly forever, and the test made the
wrong answer permanent. A belt scanning a subset is a partial belt, and it looks
exactly like a whole one.

`--account-repos` is **not** the CWD inference #594 removed. That inferred the
repo set from where the command happened to run, which is why it silently
narrowed. This reads it from a declared manifest, and **fails hard** if the
manifest cannot be read or declares nothing for the account — an empty repo set
reads as "no work anywhere", the one reading this protocol exists to refuse. A
declared checkout that is not present is reported and skipped, not fatal: not
...
discover_queue l3  [ready-for-l3, disc]  -> {}          # discussion EVICTS
discover_queue l3  [ready-for-l3, find]  -> {1530: 'ready-for-l3'}  # finding does not
```

| filter | applies to | why it exists |
|---|---|---|
| `QUEUE_KINDS[lane]` | `discover_queue` only | the newest comment must be a kind this lane owes work on. Eviction is the point: posting anything else clears the queue, which is what makes it self-clearing with no "done" bookkeeping |
| `_is_l2_finding` | `discover_queue` only | harmonic-forge#580 AC1. Without it a `## L2 Finding` after a `ready-for-l3` marker made the newest comment Lane 2's, evicting a genuinely queued issue from Lane 3's belt |
| `discussion` removed from `QUEUE_KINDS["l2"]` | `discover_queue`, Lane 2 | measured: 63 issues whose newest post-`l2.done` marker was a `discussion`, none actionable. Removing it lets those evict rather than linger |

**Do not "simplify" these away.** An out-of-family review caught exactly that
proposal: deleting `_is_l2_finding` reintroduces #580's false retraction. Each
